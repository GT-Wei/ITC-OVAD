...
class CocoDetection(torchvision.datasets.CocoDetection):
    ...
    def __getitem__(self, idx):
        ...
        if 'boxes' in target:
            # Convert the boxes to the desired format if necessary (XYXY)
            target['boxes'] = torch.tensor(target['boxes'])  # Ensure it's a tensor
            target['boxes'] = box_convert(target['boxes'], in_fmt='xywh', out_fmt='xyxy')

        if 'masks' in target:
            # Convert masks (if they are not already in the required format)
            target['masks'] = torch.tensor(target['masks'])  # Ensure it's a tensor

...
#             backend='nccl',
#             init_method='env://',
#             timeout=timedelta(seconds=3600)  # <-- 设置超时时间为 300s
#         )
...
def init_distributed(backend: str = "nccl", timeout_sec: int = 36000):
    """
    支持 torchrun 与单进程两种启动方式。
    无论哪种方式，都可安全重复调用而不会报错。
    """
    if tdist.is_initialized():          # 已经初始化就直接返回
        return True
    ...
    # ③ 只在 rank0 打印一次
    if rank == 0:
        print(f"[DDP] world_size={world_size}  backend={backend}")
